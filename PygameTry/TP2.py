...
############################ HIGH SCORE?? ###############################
    ...
    def winChoices(self,event):
        if self.winchoice != None:
            ...
            if self.winchoice == 0:
                ...
                self.init(self.levelChosen)
            elif self.winchoice == 2:
                self.levelCleared = range(self.levelChosen+1)
                if self.levelChosen != 7:
                    self.levelChosen += 1
                    self.init(self.levelChosen)
                else:
                    self.doMainMenu()
    ...
    def identifyLevelImg(self,event):
        ...
        if self.levelPage == "1-3":
            ...
            if len(self.levelCleared[1:]) >= 3:
                self.actLevel4_6(x,y)
            else:
                self.unactLevel4_6(x,y)
        else:
            if len(self.levelCleared[1:]) >=6:
                self.actLevel7(x,y)
            else:
    ...
    def level2Text(self):
        if self.animateCount < 60:
            note = "Welcome to level 2. You have learned how to control, now try!"
        else:
            note = "Enjoy!"
    ...
    def AIAssist(self,cell):
        ...
        try:
            ...
            for allyInfo in alliesList:
                ally = allyInfo[2]
                if self.dic[ally] != -1:
                    for i in xrange(len(self.dic[ally])):
    ...
    def inBreakProcess(self,cell,chainEnd,currentChain):
        ...
        if (validLow + validHigh) % 2 == 1:
            if validHigh != 0 and chainEnd.value < self.maximum:
                ...
                if chainEnd.value < 1:
                    if chainEnd.color == GRAY:
                        chainEnd.value = 10 # bonus for gray!
                    else:
                        self.forceMakeCollapse(chainEnd)
                        self.enemyKilled += 1
                        chainEnd.value = abs(chainEnd.value)
    ...
    def redrawAll(self):
        ...
        needlex,needley = 647,643
        self.screen.blit(self.needleImg,(needlex,needley))
        font = pygame.font.SysFont("Franklin Gothic Demi",25,False)
        textObj = font.render("%d x "%self.needleLeft,True,GREEN)
        self.screen.blit(textObj,(610,650))
    ...
    def finalLevel1_3(self):
        ...
        textObj1 = font1.render("Enter the level(Keyboard): %s"%text,\
                                False,(2,2,2))
        self.screen.blit(textObj1,(110,420))
        ...
        if len(text) == 1 and ((eval(text) not in tempLevelCleared)\
           or eval(text) > 3):
            ...
            self.screen.blit(textObj2,(220,470))
        pygame.display.update()
    ...
    def finalLevel4_6(self):
        ...
        textObj1 = font1.render("Enter the level(Keyboard):  %s"%text,\
                                False,(2,2,2))
        self.screen.blit(textObj1,(160,420))
        tempLevelCleared = self.levelCleared + [self.levelCleared[-1]+1]
        if len(text) == 1 and ((eval(text) not in tempLevelCleared)\
    ...
    def doAchievement(self):
        ...
        self.screen.blit(textObj1,(503,149))
        self.screen.blit(textObj2,(503,205))
        self.screen.blit(textObj3,(503,264))
        self.screen.blit(textObj4,(503,323))
        self.screen.blit(textObj5,(503,486))
        pygame.display.update()
    ...
    def doAchievementPage2(self):
        ...
        title = []
        if totalMerge >= 10:title.append(1)
        if totalAssist >= 5: title.appned(2)
        if enemyKilled >= 30: title.append(3)
    ...
    def init(self,level): # level as a number
        ...
        self.levelList = [Level_1(),Level_2(),Level_3(),Level_4(),Level_5(),\
                          Level_6(),Level_7()]
        self.levelChosen,self.AIEMB = level,None
...
# 4. Haven't done self.totalAssist!
# 5. Quick Button to win/lose for demo purpose.
